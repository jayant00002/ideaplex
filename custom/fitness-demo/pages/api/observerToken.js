...
export default async function handler(req, res) {
    const { roomName, isObserver } = req.body;
  
    if (req.method === 'POST' && roomName) {
      console.log(`Getting token for room '${roomName}' as observer: ${isObserver}`);
      ...
      const options = {
        ...
        body: JSON.stringify({
          properties: { room_name: roomName, is_observer: isObserver },
        }),
      };
