...
export const ScreensAndPins = ({ items }) => {
  ...
  const visibleItems = useDeepCompareMemo(() => {
    let item ;
    const isPinnedScreenshare = ({ id, isScreenshare, hasNameSet }) =>
      isScreenshare && !hasNameSet;
    if (items.some(isPinnedScreenshare)) {
      console.log(items.filter(isPinnedScreenshare));
       items.filter(isPinnedScreenshare);
       item = items.filter(isPinnedScreenshare);
    }
    const isPinned = ({ id }) => id === pinnedId;
    if (items.some(isPinned)) {
      console.log(items.filter(isPinned));
      items.filter(isPinned);
      item = items.filter(isPinned);
    }
    console.log(items)
    return item;
  }, [items, pinnedId]);
