...
import { useLocalSessionId } from '@daily-co/daily-react-hooks';
...
import { ReactComponent as RaiseHand } from '@custom/shared/icons/raiseHand.svg';
import { ReactComponent as LowerHand } from '@custom/shared/icons/lowerHand.svg';
...
export const BasicTray = () => {
  ...
  const linkNo = callObject.participants()['local'].userData.linkNo;
 ...
 const localParticipantHasHandRaised = handRaisedParticipants?.includes(localSessionId);
  ...
  const leftMeeting = async () => {
      try{
        const res = await fetch(
          'https://ipex-backend-kohl.vercel.app/api/links/leave',
          {
            method: 'POST',
            headers: {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (ref.current && !ref.current.contains(event.target))
        setShowMore(false);
      if(localParticipant.isObserver)
      {
