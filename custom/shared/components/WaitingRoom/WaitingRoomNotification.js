...
import {participantReducer,initialParticipantsState} from '../../contexts/participantsState';
import Button from '../Button';
import { Card, CardBody, CardFooter } from '../Card';

export const WaitingRoomNotification = () => {
  const { callObject } = useCallState();
  const [state, dispatch] = useReducer(participantReducer, initialParticipantsState);
  ...
  useEffect(() => {
    ...
    const handleWaitingParticipantAdded = () => {
      ...
      if(showNotification) playMessageSound();
    };
  ...
  }, [callObject, showModal, playMessageSound, showNotification]);
