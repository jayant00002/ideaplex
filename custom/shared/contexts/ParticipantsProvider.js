...
export const ParticipantsProvider = ({ children }) => {
  ...
  const participantCount = useMemo(
    //if isObserver is true and screenshare is on, then filter out the participant
    () =>
      [...state.participants, ...state.screens].filter(
        (p) => !p?.isObserver
      ).length,
    [state.participants, state.screens]
  );
  ...
  const isObserver = useMemo(() => !!localParticipant?.isObserver, [
    localParticipant,
  ]);
  ...
  async function updateLeftParticipant(participant)
  {
    ...
    try{
      const res = await fetch(
        'https://ipex-backend-kohl.vercel.app/api/links/leave',
        {
          ...
          body: JSON.stringify({
            roomName: room.name,
            linkNo: participant.userData.linkNo,
          }),
        }
      );
    ...
    (event = null) => {
      switch (event?.action) {
        ...
        case 'participant-left':
          ...
          updateLeftParticipant(event.participant);
          break;
      }
    },
